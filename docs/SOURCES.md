...
取得・確認日：2026年9月5日。各風景写真を縮小・WebP変換し、色抽出、横方向のぼかし、粒状感の追加、切り抜き表示に使用しています。写真と派生する画像・色帯には、各写真の元のライセンスが適用されます。画像の作者はアプリの推奨・協賛者ではありません。
...
- 提供素材 `image (1).png` を `public/window/frame.png` にコピー。元写真の画素は変更せず、開口部を `frame-mask.svg` で表示時にマスクしています。
- この提供写真に、上記Wikimedia Commons写真のライセンスを適用したものではありません。
- [窓枠の制作記録](WINDOW_ASSET.md)。その他のスケッチ画像は `reference/` に保管し、リポジトリや配信成果物には含めていません。
